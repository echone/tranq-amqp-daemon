...
#	 $title = $titolo." [Audio]"; //Title for your RSS feed
	 ...
	 $description = $descrizione." [File video in formato AVI]";	//Brief description of your feed
	 $link_base   = $url_base."/";	//Base URL for linking files (INCLUDE ENDING SLASH!!!)
	 $feed_url    = $url_base."/rss_hd.php";	//Full URL to the rss.php file
	 ...
	 $rss_file_name = "rss_hd.php";	//This needs to be changed if you are changing the name of this file
		...
		if (is_dir($dir)) {
			if ($dh = opendir($dir)) {
				...
				foreach($contents as $k=>$v) {
					...
					?>
						<item>
							<title><?php echo str_replace('.avi','',$file); ?></title>
   		                     <itunesu:category itunesu:code="<?php echo stripslashes($categoria); ?>"/>
							<link><?php echo $link_base.str_replace(' ','%20',$file); ?></link>
                            <guid><?php echo $link_base.str_replace(' ','%20',$file); ?></guid>
                            <enclosure type="video/avi" length="<?php echo filesize($file); ?>" url="<?php echo $link_base.str_replace(' ','%20',$file); ?>" />							<pubDate><?php echo $data_pub ; ?></pubDate>
							<description>File AVI: (<?php echo round(filesize($file)/1024/1024,1); ?> MB)</description>
							<itunes:duration>
							<?php
