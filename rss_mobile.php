...
#	 $title = $titolo." [Audio]"; //Title for your RSS feed
	 ...
	 $description = $descrizione." [File video in formato MP4]";	//Brief description of your feed
	 $link_base   = $url_base."/";	//Base URL for linking files (INCLUDE ENDING SLASH!!!)
	 $feed_url    = $url_base."/rss_mobile.php";	//Full URL to the rss.php file
	 ...
	 $rss_file_name = "rss_mobile.php";	//This needs to be changed if you are changing the name of this file
...
<rss version="2.0" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:itunesu="http://www.itunesu.com/feed">
	...
	<channel>
        ...
        <itunesu:category itunesu:code="<?php echo stripslashes($categoria); ?>"/>
		...
		if (is_dir($dir)) {
			if ($dh = opendir($dir)) {
				...
				foreach($contents as $k=>$v) {
					...
					?>
						<item>
							<title><?php echo str_replace('.mp4','',$file); ?></title>
   		                     <itunesu:category itunesu:code="<?php echo stripslashes($categoria); ?>"/>
							<link><?php echo $link_base.str_replace(' ','%20',$file); ?></link>
                            <guid><?php echo $link_base.str_replace(' ','%20',$file); ?></guid>
                            <enclosure type="video/mp4" length="<?php echo filesize($file); ?>" url="<?php echo $link_base.str_replace(' ','%20',$file); ?>" />							<pubDate><?php echo $data_pub ; ?></pubDate>
							<description>File MP4: (<?php echo round(filesize($file)/1024/1024,1); ?> MB)</description>
							<itunes:duration>
							<?php
							$mb=round(filesize($file)/1024,1);
							$min=round($mb*8/$br_hd);
							echo $min;
							?>
